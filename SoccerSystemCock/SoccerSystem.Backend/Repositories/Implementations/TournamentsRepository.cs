...
public class TournamentsRepository : GenericRepository<Tournament>, ITournamentsRepository
{
    ...
    public async Task<ActionResponse<Tournament>> AddAsync(TournamentDTO tournamentDTO)
    {
        ...
        if (!string.IsNullOrEmpty(tournamentDTO.Image))
        {
            ...
            tournament.Image = await _fileStorage.SaveFileAsync(imageBase64, ".jpg", "tournaments");//ojo
        }
    ...
    {
        return await _context.Tournaments.Where(x => x.IsActive)
                                         .OrderBy(x => x.Name)
                                         .ToListAsync();
    }
    ...
    {
        var tournament = await _context.Tournaments.Include(x => x.TournamentTeams!)
