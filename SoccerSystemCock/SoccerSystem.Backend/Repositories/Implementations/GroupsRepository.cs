...
public class GroupsRepository : GenericRepository<Group>, IGroupsRepository
{
    ...
    public override async Task<ActionResponse<IEnumerable<Group>>> GetAsync(PaginationDTO pagination)
    {
        var queryable = _context.Groups
                                ...
                                .AsQueryable();
        queryable = queryable.Where(x => x.Members!.Any(x => x.User.Email == pagination.Email));//ojo mira liquion
    ...
    public async Task<ActionResponse<Group>> AddAsync(GroupDTO groupDTO)
    {
        ...
        //codigo evita el codigo se repita, aunque es poco probable que suceda, pero por si acaso, se genera un nuevo codigo hasta que no exista en la base de datos
        do
        {
    ...
    public async Task<ActionResponse<Group>> UpdateAsync(GroupDTO groupDTO)
    {
        ...
        if (currentGroup == null)
        {
            ...
            {
                WasSuccess = false,
                Message = "ERR014"// grupo no existe
            };
        }
        ...
        catch (DbUpdateException)
        {
            ...
            {
                WasSuccess = false,
                Message = "ERR003"// error al actualizar el registro
            };
        }
        catch (Exception exception)
        {
