...
public class PredictionsRepository : GenericRepository<Prediction>, IPredictionsRepository
{
    ...
    {
        //[ThenInclude(x => x.Local)..ThenInclude(x => x.Local)] ojo para consultar la tabla equipo se tiene consultar el match despues el team para el local depuess
        // toca otra ves el match y despues el team para el visitante
        ...
        {
            WasSuccess = true,
            Result = await queryable.OrderBy(x => x.Match.IsClosed)
